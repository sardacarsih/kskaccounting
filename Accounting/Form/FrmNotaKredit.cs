...
{
    public partial class FrmNotaKredit : DevExpress.XtraEditors.XtraForm
    {

       
        string pperiode;
        public FrmNotaKredit()
        {
            InitializeComponent();
        }
        ...
        DataSet _ds;
        private void Load_JurnalNotaKredit()
        {
            ...
            gridViewND.Columns[6].Summary.Add(DevExpress.Data.SummaryItemType.Sum, "KREDIT", "{0:N2}");
            gridViewND.BestFitColumns();
           
        }

        private void Load_AkunNotaKredit()
        {
            var PPERIODE = (cmbbulan.SelectedIndex + 1).ToString("00") + "/" + setahun.Value;
            ...
            string query = "select DISTINCT J.KODE,A.NAMAACC PERKIRAAN FROM ACCT_JURNAL_DTL J " +
                ...
                "WHERE J.IDDATA =:PIDDATA AND J.PERIODE =:PPERIODE AND SUBSTR(J.KODE, 0, 3) = '12.' AND J.KREDIT>0 " +
        ...
        private void sbtandai_Click(object sender, EventArgs e)
        {
            ...
            if (belumdikirim == 0)
            {
                XtraMessageBox.Show("Semua Nota Kredit telah dikirim", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void btncetak_Click(object sender, EventArgs e)
        {
            ...
            var x = gridView2.GetRowCellValue(rowhandle, "PERKIRAAN").ToString();
            var kepada =x;
            //_ds.WriteXmlSchema("NotaKredit.xsd");
            NotaKredit laporan = new NotaKredit
            {
                DataSource = _ds
            };
