-- Adds display level metadata matching the old ACC_SETUPLABARUGI LVL behavior.
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(1)
    ...
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE ACCT_REPORT_SECTION ADD DISPLAY_LVL NUMBER(3) DEFAULT 1 NOT NULL';
    END IF;

    EXECUTE IMMEDIATE q'[
        UPDATE ACCT_REPORT_SECTION
           SET DISPLAY_LVL = CASE SECTION_CODE
               WHEN 'PENDAPATAN' THEN 4
               WHEN 'HPP' THEN 3
               WHEN 'BIAYA_UMUM_ADMIN' THEN 2
               WHEN 'PENDAPATAN_DILUAR_USAHA' THEN 4
               WHEN 'PENGELUARAN_DILUAR_USAHA' THEN 4
               WHEN 'BIAYA_TM' THEN 1
               ELSE NVL(DISPLAY_LVL, 1)
           END
...
END;
/
-- Purpose: Restore Laba Rugi display-level behavior and account labels for metadata report engine.
...
END ACCT_REPORT_ENGINE_V1;]';
    ...
    DBMS_OUTPUT.PUT_LINE('FIXED LABARUGI DISPLAY LEVEL AND LABELS');
END;
/